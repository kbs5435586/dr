...
HRESULT CDevice::CreateDescriptorHeap()
{
	...
	DescHeap.Type = D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV;
	DescHeap.Flags = D3D12_DESCRIPTOR_HEAP_FLAG_SHADER_VISIBLE;
	if (FAILED(m_pDevice->CreateDescriptorHeap(&DescHeap, IID_PPV_ARGS(&m_pSrvDescriptorHeap))))
		return E_FAIL;
	m_iSrvDescriptorIncrementSize = m_pDevice->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
