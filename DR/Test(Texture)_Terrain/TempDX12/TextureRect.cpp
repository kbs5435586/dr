...
void CTextureRect::Render_GameObject()
{
	...
	{
		m_pShaderCom->SetUp_OnShader(m_pPipeLine, m_pConstBuffer, matWorld, matView, matProj, tMainPass, ROOT_TYPE_TEXTURE);
...
HRESULT CTextureRect::CreatePipeLine(CShader* pShader)
{
	...
	d3dPipelineStateDesc.pRootSignature = CDevice::GetInstance()->GetRootSignature(ROOT_TYPE_TEXTURE);
	//d3dPipelineStateDesc.pRootSignature = CDevice::GetInstance()->TempRoot;
