...
HRESULT CRect::Ready_GameObject()
{
	if (FAILED(Ready_Component()))
		return E_FAIL;
	if (FAILED(CreateConstantBufferDesc()))
		return E_FAIL;
	if (FAILED(CreateConstantBufferView()))
		return E_FAIL;
...
void CRect::Render_GameObject()
{
	...
	if (nullptr != m_pShaderCom)
		m_pShaderCom->SetUp_OnShader(m_pPipeLine, matWorld, matView, matProj, tMain);
	mObjectCB->CopyData(0, tMain);
	if (nullptr != m_pBufferCom)
		m_pBufferCom->Render_VIBuffer();

	CDevice::GetInstance()->GetCommandList()->SetGraphicsRootDescriptorTable(0, mCbvHeap->GetGPUDescriptorHandleForHeapStart());
...
HRESULT CRect::CreateConstantBufferDesc()
{
	...
	if (FAILED(CDevice::GetInstance()->GetDevice()->CreateDescriptorHeap(&cbvHeapDesc,
...
HRESULT CRect::CreateConstantBufferView()
{
	mObjectCB = std::make_unique<UploadBuffer<MAINPASS>>(CDevice::GetInstance()->GetDevice(), 1, true);

	UINT objCBByteSize = CalcConstantBufferByteSize(sizeof(MAINPASS));
...
void CRect::Free()
{
	...
	Safe_Release(mCbvHeap);
	mObjectCB->~UploadBuffer();
	CGameObject::Free();
}
