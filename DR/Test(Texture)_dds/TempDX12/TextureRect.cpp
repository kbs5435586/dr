...
void CTextureRect::Render_GameObject()
{
	...
	{
		m_pShaderCom->SetUp_OnShader(m_pPipeLine, m_pConstBuffer, matWorld, matView, matProj, tMainPass);
	...
	if (nullptr != m_pBufferCom && nullptr != m_pConstBuffer && nullptr!=m_pTextureCom)
	{
		...
		CDevice::GetInstance()->GetCommandList()->SetDescriptorHeaps(1, &heap);
		CDevice::GetInstance()->GetCommandList()->SetGraphicsRootDescriptorTable(0, hTexture);
...
HRESULT CTextureRect::CreateShaderResourceView()
{
	...
	srvDesc.Format = m_pTextureCom->GetTexture()->GetDesc().Format;
	srvDesc.ViewDimension = D3D12_SRV_DIMENSION_TEXTURE2D;
	srvDesc.Texture2D.MostDetailedMip = 0;
	srvDesc.Texture2D.MipLevels = m_pTextureCom->GetTexture()->GetDesc().MipLevels;
	srvDesc.Texture2D.ResourceMinLODClamp = 0.0f;

	CDevice::GetInstance()->GetDevice()->CreateShaderResourceView(m_pTextureCom->GetTexture(), 
		&srvDesc, hDescriptor);
...
HRESULT CTextureRect::Ready_Component()
{
	...
	m_pTextureCom = (CTexture*)pManagement->Clone_Component(SCENE_LOGO, L"Component_Texture_Default");
	NULL_CHECK_VAL(m_pShaderCom, E_FAIL);
