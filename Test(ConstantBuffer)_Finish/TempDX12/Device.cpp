...
HRESULT CDevice::OnCreate()
{
	...
	m_pConstBuffer->SetName(L"Constant Buffer View");
...
HRESULT CDevice::CreateConstantView()
{
	_int iSize = CalcConstantBufferByteSize(sizeof(MAINPASS));

	if (FAILED(m_pDevice->CreateCommittedResource(&CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_UPLOAD),
	...
	m_pConstBuffer->Map(0, nullptr, reinterpret_cast<void**>(&m_pMappedData));

	UINT iObjectSize = CalcConstantBufferByteSize(sizeof(MAINPASS));

	D3D12_GPU_VIRTUAL_ADDRESS cbAddress = m_pConstBuffer->GetGPUVirtualAddress();

	int boxCBufIndex = 0;
	cbAddress += boxCBufIndex * iObjectSize;
...
void CDevice::Begin()
{
	...
	//m_pCommandList->SetGraphicsRootDescriptorTable(0, m_pCbvDescriptorHeap->GetGPUDescriptorHandleForHeapStart());
}
